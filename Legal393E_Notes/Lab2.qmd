---
title: "Legel 393E Spring 2025: Lab Meeting#2: Tutorial#2 Quiz Solutions & More on R Wrangling"
author: "EricoYu"
...
Note: if I know what packages I want to load for a particular project, I can load them all at once:

```{r}
needed.packages<-c('readxl','readr','haven',"dplyr") #define a vector of the names of needed pcakges, and assign it a name of "needed.package" 

lapply(needed.packages, require, character.only = TRUE) #lapply() function allows us to apply and execute every included elements in an object.
...
![](images/clipboard-3712000153.png)
...
```{r}
#If you store your data in a cloud drive, R may not be able to read the file (it is not sure whether it stored locally or online)

library(readr)
electric <- read_csv("C:/Users/donge/OneDrive/Desktop/GitHub/GitHub_ELS/Course_Empirical_Legal_Studies/data/API_EG.ELC.ACCS.ZS_DS2_en_csv_v2_76137.csv", 
    skip = 3)
...
```{r}
#using dplyr packages (a tidyverse family package)to help to edit and change dataset. Since we already load it above, we don't have to load it here.
#library(dplyr)
...
Answer: Based on tutorial#2, we can just use table() the get the answer.
...
But we can also use the filter() function to keep only the rows of the school, so you don't have to browse the table and find the number yourself.
...
### Data Structure and Object Types: [Reading](https://r4ds.had.co.nz/vectors.html)

**#Object Types** **1. Please create the four different types of atomic vectors. Make sure they at least have four elements (length = 4). You can double-check their types by function typeof()**

`{r} logical <- c(TRUE, FALSE, FALSE)  integer<-c(3, 10)  double <- c(1.5, 2.8, pi)  character <- c("this", "is", "a character", "vector")}`
...
`{r} #option 1: new_integer<-c(3L, 10L) typeof(new_integer)  #option 2: coercion_integer<-as.integer(integer) typeof(coercion_integer)}`
...
`{r} #the first element double[1]  #the second to third element double[2:3]  #the last element tail(double, 1)}`

3.  **Let's create a list combing the above four atomic vectors.**

`{r} my_list<-list(integer, double, character, logical) my_list}`

4.  **Next, create a simple n by n tibble of your own. You can use the previous vectors you created above.**

`{r} my_tibble<-tibble(logical, double) my_tibble}`
...
2.  **Now, load the dataset "mpg": data(dataset_name);**
